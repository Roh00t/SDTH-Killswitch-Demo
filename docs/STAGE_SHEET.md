...
| ☐ | Camera address | `python -m tools.serial_probe --port COM3 --interactive`. Wait about 10 s | `Firmware: OK BOOT killswitch-actuator v3.1 … pwm=ok`, `CAM camera ok, sensor 0x5640`, `CAM wifi "<name>" heard on channel …`, then `Firmware: CAM http://<ip>/stream`. Put that URL in `camera.stream_url` in `config/fallback.yaml`. `CAM FAIL` means check the ribbon and the PSRAM setting. `NOT FOUND` means the hotspot is 5 GHz or the name differs; `CAM wifi not joined: …` repeats every 10 s and names the reason |
